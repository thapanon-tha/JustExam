<template>
  <div class="mb-60">
    ...
    <Countdown class="mb-10" v-bind:endTime="'2022-12-12T11:16:43.000Z'" v-bind:submit="onSubmit" />
...
export default {
  ...
  methods: {
    ...
    checkAnswerExam() {
      ...
      if (notFinish.length > 0) this.textFinish = `คุณยังไม่ได้ทำข้อสอบข้อที่ ${notFinish.map((value) => value + 1).join(', ')}`;
      else this.onSubmit();
    },
    onSubmit() {
      this.finish = true;
