...
const medthods = {
  ...
  async createMany(answers, qid, qtid, transaction) {
    return Promise.all(answers.map((answer) => medthods.createModel(answer, qid, qtid, transaction)));
  },
  ...
  deleteAnswerByQid(qid, transaction) {
    return [questionAnswerTF.destroy({ where: { qid } }, { transaction }), questionAnswerC.destroy({ where: { qid } }, { transaction }), questionAnswerSA.destroy({ where: { qid } }, { transaction }), questionAnswerM.destroy({ where: { qid } }, { transaction }), questionAnswerMC.destroy({ where: { qid } }, { transaction })];
  },
