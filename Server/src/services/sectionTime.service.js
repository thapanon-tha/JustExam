...
const medthods = {
  ...
  async createMany(datas, transaction) {
    return Promise.all(datas.map((data) => medthods.createModel(data.ecid, data.sectionName, data.time, transaction)));
  },
  ...
  async updateMany(datas, transaction) {
    return Promise.all(datas.map((data) => medthods.updateTime(data.setcid, data.sectionName, data.time, transaction)));
  },
