...
module.exports = {
  async getQuestion(req, res) {
    ...
    try {
      ...
      const datas = await questions.map((data) => {
        ...
        if (data.questionAnswerCs.length !== 0) {
          finalData.answer = data.questionAnswerCs;
        }
        if (data.questionAnswerMCs.length !== 0) {
          finalData.answer = data.questionAnswerMCs;
        }
        if (data.questionAnswerSAs.length !== 0) {
          finalData.answer = data.questionAnswerSAs;
        }
        if (data.questionAnswerTFs.length !== 0) {
          finalData.answer = data.questionAnswerTFs;
        }
        if (data.questionAnswerMs.length !== 0) {
          finalData.answer = data.questionAnswerMs;
        }
        return finalData;
      });
  ...
  async updateQueation(req, res) {
    ...
    try {
      ...
      const resultUpdate = await Question.updateQuestion(
        eid,
        qid,
        questionTopic,
        sectionName,
      ...
      if (resultUpdate[0]) {
        await transaction.commit();
        const data = await Question.getQuestionById(qid, eid, uid).then(
          (responses) => responses.map((response) => {
            ...
            if (response.dataValues.questionAnswerCs.length !== 0) {
              finalData.questionAnswer = response.dataValues.questionAnswerCs;
            }
            if (response.dataValues.questionAnswerMCs.length !== 0) {
              finalData.questionAnswer = response.dataValues.questionAnswerMCs;
            }
            if (response.dataValues.questionAnswerSAs.length !== 0) {
              finalData.questionAnswer = response.dataValues.questionAnswerSAs;
            }
            if (response.dataValues.questionAnswerTFs.length !== 0) {
              finalData.questionAnswer = response.dataValues.questionAnswerTFs;
            }
            if (response.dataValues.questionAnswerMs.length !== 0) {
              finalData.questionAnswer = response.dataValues.questionAnswerMs;
            }
